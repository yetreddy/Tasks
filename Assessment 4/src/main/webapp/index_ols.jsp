    ...
    <div class="tab-content mt-2">
            ...
            <div class="container mt-3">
		 	...
		 	<div id="content">
		 		<table border = "3" width = "100%">
		 			...
		 			<tr>
		 			<td> <a href="#" id="myBtn" onclick="test1('${tempStudent1.parkingReason}','${tempStudent1.simLink}');">${tempStudent.taskName}</a></td>
		 			<td> ${tempStudent.status} </td>
		 			<td> ${tempStudent.startDate}</td>
		 			<td> ${tempStudent.endDate}</td>
		 			<td><button type="button" class="btn btn-secondary btn-block" id="myBtn2">Delete</button></td>
		 			</tr>
		 			</c:forEach>
		 		</table>
		 	</div>
        ...
        <div class="tab-pane fade" id="completedTasks">       
            ...
            <c:forEach var="tempStudent1" items="${tasksList1}">
		 			...
		 			<td id="completedTaskName"> <a href="#" id="completeTaskDialog" onclick="test('${tempStudent1.parkingReason}','${tempStudent1.simLink}','${tempStudent1.startDate}','${tempStudent1.endDate}');">${tempStudent1.taskName}</a></td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <!-- Form for adding tasks -->                    
                    ...
                    <!-- Save Button -->
                    <button type="button" class="btn  btn-outline-success float-right" id="saveTaskBtn">Create New Task</button>
               </div>
            </form>          
        </div>
    </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">   
          <div class="form-group">
                            ...
                            <select class="form-control" id="taskStatus1" name="taskStatus1" readonly="readonly">
                        ...
                        <div class="form-group" id="parkedReasonDiv1">
                            <label for="parkedReason">Reason for Parking</label>
                            <textarea class="form-control" id="parkingReason" name="parkingReason" rows="3" readonly></textarea>
                        </div>
      ...
      <div class="modal-content">
        ...
        <c:forEach var="tempStudent" items="${tasksList}">
                    <input type="hidden" name="tasksId" value="${tempStudent.id}" />
                        ...
                        <button type="button" class="btn btn-primary float-right" id="deleteBtn">Delete</button>
                        </div>
                        </form>
        </div>
                ...
                <div class="modal-body">
                   ...
                   <c:forEach var="tempStudent" items="${tasksList}">
                    <input type="hidden" name="tasksId" value="${tempStudent.id}" />
                        ...
                        <!-- Update Button -->
                        <button type="button" class="btn btn-outline-success float-right" id="performUpdateBtn">Update Details</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
 ...
 /* $(".modal-body #parkedReason").val( parkingReason );
	...
	// Show/hide the parked reason textarea based on selected status
	 ...
	 function test(parkingReason, simLink,startDate,endDate) {
		 console.log(simLink)
    ...
    $("#completedTask").modal();
	 ...
	 function test1(parkingReason, simLink ) {
		 /* console.log(simLink)
		 console.log(parkingReason) */
 	$(".modal-body #SIMLink").val(simLink);
	$(".modal-body #parkingReason").val(parkingReason); 
